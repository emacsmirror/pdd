...
(ert-deftest pdd-test-cacher-key ()
  (should (equal (pdd-cacher-key "~/" '(a b c)) "6ef2031afe7508735e243afd96fbe4b3"))
  (should (equal (pdd-cacher-key "~/" "hello.") "hello."))
  (should (equal (pdd-cacher-key (make-hash-table) '(a b c)) '(a b c)))
  (should (equal (pdd-cacher-key (make-hash-table) "hello.") "hello."))
  (let ((c1 (pdd-cacher :keys (lambda () "abc") :store (make-hash-table)))
        (c2 (pdd-cacher :keys (lambda () "abc") :store "~/"))
        (c3 (pdd-cacher :keys (lambda (k) (cons 1 k)) :store (make-hash-table)))
        (c4 (pdd-cacher :keys (lambda (k) (concat "-" k)) :store "~/"))
        (c5 (pdd-cacher :keys '(xxx (data . b)) :store (make-hash-table)))
        (c6 (pdd-cacher :keys '(xxx (data . b)) :store "~/")))
    (should (equal (pdd-cacher-key c1 '(a b c)) "abc"))
    (should (equal (pdd-cacher-key c2 '(a b c)) "abc"))
    (should (equal (pdd-cacher-key c3 '(a b c)) '(1 a b c)))
    (should (equal (pdd-cacher-key c4 "hello.") "-hello."))
    (should (equal (pdd-cacher-key c5 (pdd-request :backend pdd-backend :url "kk" :data '((a . 1) (b . 2)))) '(xxx 2)))
    (should (equal (pdd-cacher-key c6 (pdd-request :backend pdd-backend :url "kk" :data '((a . 1) (b . 2)))) "f341eda5609d62686e569ef42819865b"))))
  ...
  (let ((dir (make-temp-file "pdd-tests-cacher-" t)))
    (unwind-protect
        (let* ((ht (make-hash-table :test #'equal))
               ...
               (c1 (pdd-cacher :store ht :keys '(xxx (data . b))))
               (c2 (pdd-cacher :store dir :keys '(xxx (data . b)))))
